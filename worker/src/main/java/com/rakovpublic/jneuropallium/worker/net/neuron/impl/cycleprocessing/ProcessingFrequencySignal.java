...
public class ProcessingFrequencySignal extends AbstractSignal<ProcessingFrequencySignalItem> implements ISignal<ProcessingFrequencySignalItem> {
    ...
    public ProcessingFrequencySignal copySignal() {
        return new ProcessingFrequencySignal(value, this.getSourceLayerId(), this.getSourceNeuronId(), this.getTimeAlive(), getDescription(), isFromExternalNet(), getInputName(), this.isNeedToRemoveDuringLearning(), this.isNeedToProcessDuringLearning(), this.getName());
    }
}
