...
public class SimpleLearningAlgo implements IDirectLearningAlgorithm {
    @Override
    public List<ILearningRequest> learn(IStructMeta structMeta, Long neuronId) {
        List<ILearningRequest> result = new LinkedList<>();
    ...
    private void rebuildConnection(Long neuronId, int layerId, IStructMeta structMeta, Long sourceNeuron, List<ILearningRequest> result) {
        List<ISignal> resultSignals = structMeta.getInputResolver().getSignalPersistStorage().getLayerSignals(layerId).get(neuronId);
        ...
        layer.ifPresent(l -> {
            l.getNeuronByID(neuronId).getAxon().changeAllWeightsForNeuron(layerId + 1, sourceNeuron, new SimpleChangeWeightSignal(0.5d, layerId, neuronId));
