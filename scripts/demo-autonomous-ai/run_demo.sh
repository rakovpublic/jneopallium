...
"$MVN" -q -pl demos/demo-autonomousai -am -DincludeScope=runtime dependency:build-classpath -Dmdep.outputFile=target/autonomous-ai-classpath.txt

WORKER_JAR="$ROOT_DIR/demos/demo-autonomousai/target/demo-autonomousai-1.0-SNAPSHOT.jar"
DEPS="$(cat "$ROOT_DIR/demos/demo-autonomousai/target/autonomous-ai-classpath.txt")"
PATH_SEPARATOR=":"
case "$(uname -s)" in
  MINGW*|MSYS*|CYGWIN*)
    PATH_SEPARATOR=";"
