...
| [Simulation-Only Single UAV](docs/demo-uav-single.md) | Single-UAV observation demo with search-area coverage, CNN-style pixel-matrix image recognition, autonomous and target-confirm modes, transparent target priority scoring, confirmation binding, simulator-only mission supervision, safety vetoes, simulated photography metadata, and deterministic artifacts. | `scripts/demo-uav-single/run_demo.sh all` or `scripts/demo-uav-single/run_demo.ps1 all` |
