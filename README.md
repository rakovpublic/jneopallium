...
| [Simulation-Only Single UAV](docs/demo-uav-single.md) | Single-UAV observation demo with search-area coverage, CNN-style 3x3 patch recognition for FPV pixel matrices, simulator reinforcement feedback, autonomous and target-confirm modes, transparent target priority scoring, confirmation binding, simulator-only mission supervision, safety vetoes, simulated photography metadata, and deterministic artifacts. | `scripts/demo-uav-single/run_demo.sh all` or `scripts/demo-uav-single/run_demo.ps1 all` |
| [Industrial FMI Skid](docs/demo-industrial-fmi.md) | Heated circulation and heat-exchanger skid demo with FMI 2.0 source, OPC UA autonomous command path, MQTT advisory telemetry, Jneopallium validation/control/health supervision, safety gating, audits, and deterministic comparison artifacts. | `scripts/demo-industrial-fmi/run_demo.sh all` or `scripts/demo-industrial-fmi/run_demo.ps1 all` |
